...
impl MultiTreeFunction {
    ...
    pub fn push_new_packet(&mut self, packet: &InjectionPacket) {
        ...
        let packet = self.gen_packet(self.id, dest_vid, next_vid, packet.message.clone());
    ...
    pub fn update(&mut self) {
        ...
        if self.parent_id.is_none() && self.node_type == NodeType::Router && p < 0.1 {
            self.send_packet_buffer.push_back(MultiTreePacket {
                message: "preq".to_string(),
                packet_id: u32::MAX, // todo
                dest_id: BROADCAST_ID,
                source_id: self.id,
...
mod tests {
    ...
    fn test_process_received_packet_coordinator() {
        let mut protocol = MultiTreeFunction::new(&NodeType::Coordinator, 2);
        let rec_packet = MultiTreePacket {
            message: "preq".to_string(),
            packet_id: 0,
            dest_id: BROADCAST_ID,
            source_id: 1,
    ...
    fn test_process_received_packet_router() {
        let mut protocol = MultiTreeFunction::new(&NodeType::Router, 2);
